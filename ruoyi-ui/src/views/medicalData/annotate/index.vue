...
export default {
  ...
  methods: {
    ...
    detail(a) {
      ...
      getAnnotateData({ pId: a }).then(res => {
        ...
        this.option = Array.isArray(serverOption) ? serverOption : []
    ...
    bingliChange(val) {
      console.log(val)
      let arr = ["jzxZuzhibingliLiangxing", "jzxZuzhibingliRutouzhuangai", "jzxZuzhibingliSuiyangai", "jzxZuzhibinbliLvpaozhuangai", "jzxZuzhibingliQitaexing"]
      for (let i = 1; i <= arr.length; i++) {
        if (i == val) {
          continue
        }
    ...
    lbjbianyuanChange(val) {
      console.log(val)
      let arr = ["lbjBianyuanBuguize", "lbjBianyuanGuangzheng"]
      for (let i = 1; i <= arr.length; i++) {
        if (i == val) {
          continue
        }
  ...
  mounted() {
    ...
    getAnnotateData({ pId: this.pId }).then(res => {
      ...
      // this.option = typeof (serverOption) == 'Array' ? serverOption : []
      this.option = Array.isArray(serverOption) ? serverOption : []
      console.log('typeof (serverOption): ', typeof serverOption);
